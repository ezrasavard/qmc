...
def xfunc(x, N, a, b):
    ...
    return x/((np.log(N)*a*N)**b)

def yfunc(y, N, a, b):

    return y/((np.log(N)*a*N)**b)
...
if __name__ == "__main__":
    ...
    for row in plist:
        ...
        N = row[4]
        x = xfunc(data[:,0], N, 1, .33)
        y = yfunc(data[:,1], N, 1, .66)
    ...
    all_x = np.sort(all_x)
    plt.plot(all_x, func(all_x, *popt), c='k', linewidth=20, alpha=0.5)
    regstring = "{:.1f}*((x + {:.1f})**{:.1f}) + {:.1f}".format(*popt)
    print("Best Fit:\n\t{}".format(regstring))
    plt.xlabel("f(PTxJ, N)\nPTxJ/((N*ln(N))^0.33)")
    plt.ylabel("g(MCSxS, N)\nMCSxS/((N*ln(N))^0.66)")
#     plt.yscale('log')
    plt.legend(loc='upper left')
    plt.title("Lowest {:.0f}% of Distribution Difference\nBest Fit: {}".format(diff_percentage, regstring))
    plt.show()
